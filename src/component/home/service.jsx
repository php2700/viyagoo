...
export const OurServices = () => {
  ...
  return (
    <>
      {/* SEO FIX: Default heading mein keywords add kiye jo report mein missing the */}
      <h2 className="text-3xl md:text-4xl text-center font-bold mb-12">
        {bgData?.heading || "Our Corporate Transportation Services"}
      </h2>
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-7xl">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center">
            {services.map((service, index) => (
              ...
              >
                <img
                  src={`${API_URL}/${service.image.replace("public/", "").replace(/^\/+/, "")}`}
                  // SEO Update: Image alt ko thoda aur descriptive banaya
                  alt={`Viyagoo ${service.title} Service Icon`}
                  width="64"
                  height="64"
                  className="w-12 h-12 sm:w-16 sm:h-16 object-contain mb-2 sm:mb-4"
                  loading="lazy"
                  decoding="async" // Performance improvement for mobile score 53
                />
                <h3 className="text-sm sm:text-lg font-semibold mb-1 sm:mb-2 text-center">
                  {service.title}
                </h3>
          ...
          <div className="flex justify-center mt-10">
            <Link
              to="/ourservice#ourservice"
              // USABILITY FIX: Generic "Know More" link ke liye ARIA label add kiya
              aria-label="Learn more about our corporate transportation services"
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-3 rounded-full transition-all duration-300 shadow-lg"
            >
              Know More
