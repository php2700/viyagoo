 ...
 */}
   <section className="relative w-full min-h-[70vh] md:min-h-screen overflow-hidden">
      ...
      {/* HERO MEDIA */}
      {banner?.type === "image" || isMobile ? (
        <picture>
          {/* Mobile Image */}
          <source
            media="(max-width: 768px)"
            srcSet={`${import.meta.env.VITE_APP_URL}${banner?.mobile_image || banner?.image}`}
            type="image/webp"
          />

          {/* Desktop Image */}
          <source
            srcSet={`${import.meta.env.VITE_APP_URL}${banner?.image}`}
            type="image/webp"
          />

          <img
            src={`${import.meta.env.VITE_APP_URL}${banner?.image}`}
            alt="Viyagoo Corporate Transport Services"
            className="absolute inset-0 w-full h-full object-cover"
            loading="eager"
            fetchpriority="high"
            decoding="async"
      ...
      <div className="absolute inset-0 flex items-center justify-center text-white px-4 text-center">
        <h1 className="text-2xl sm:text-3xl md:text-5xl font-bold leading-tight max-w-5xl">
          {banner?.heading}
        </h1>
      </div>
